...
- **`TODAY_ISO` không còn tồn tại** — Phase 1 (đã DONE, merge ở `3012c42`) thay bằng ISO datetime thật (`startAt`/`endAt` + `dateKey`) và `bookingDays(anchorIso)`/`venueDays` cửa sổ 7 ngày trượt trong `shared/helpers.ts`; `today/tomorrow/sat/sun/mon` **không còn ở `shared/config.ts`** — chỉ còn sót lại như nhãn ngày trong seed data thô (`api/src/data/*.ts`), được `dayKeyForRoom()` convert sang ISO date thật lúc build seed (không phải key sống trong runtime state).
- **App booking → `pending` → venue duyệt** đã là hành vi hiện tại (`venues.service.ts:744`).
- ~~Công việc chưa commit: onboarding chọn loại tài khoản Player/Venue/Both~~ — **đã commit** (`cc3a033 Add account-type onboarding flow (player/venue/both)`, 2026-07-20): `api/src/features/account/`, `web/features/onboarding/`, redirect gate ở dashboard layout, implement đầy đủ theo TODO.md kèm test.
...
- ~~Endpoint status reservation không có state machine (status nào → status nào cũng được); decline reason chỉ bắt buộc ở web UI, optional ở API.~~ — **đã fix ở Phase 0 (DONE)**: `RESERVATION_TRANSITIONS`/`canTransitionReservation()` enforce trong `updateReservationStatus` (`ConflictException` khi sai transition); `ReservationStatusDto.reason` bắt buộc qua `@ValidateIf` khi `status === "cancelled"`.
...
- ~~Chưa enforce: court name unique, court sport ∈ venue.sports, openFrom < openTo.~~ — **đã fix ở Phase 0 (DONE)**: `venues.service.ts addCourt/updateCourt` reject trùng tên court trong venue và sport ngoài `venue.info.sports`; `createVenue`/`updateVenue` reject `openFrom >= openTo`.
...
### Phase 2 — Collection `bookings` chuẩn (quyết định #1: một record) - DONE
...
### Phase 3 — Endpoint hẹp, validate server-side (quyết định #8) - DONE
...
### Phase 4 — SePay Payment Gateway / VietQR (quyết định #3, #9) - DONE
...
- **Ghi chú triển khai (2026-07-20, cập nhật sau khi merge với Phase 2–4):** Phase 5 được viết lần đầu trên model embedded (`ops.reservations`) với giả định Phase 2–4 chưa land; khi merge vào `master` hoá ra Phase 2 (collection `bookings` chuẩn), Phase 3 (hold/checkout endpoint) và Phase 4 (SePay checkout/IPN thật) **đã land trước đó**, nên toàn bộ sweeper được viết lại trong lúc merge để chạy trên kiến trúc thật thay vì giữ bản cũ: logic ba rule sống ở `BookingsService.sweep()` (`api/src/features/bookings/bookings.service.ts`, dùng lại `setStatus`/`canTransitionBooking` sẵn có, guarded per-booking nên một booking bị race không làm hỏng cả batch); cron trigger là `BookingsSweeperService` (`api/src/features/payments/bookings-sweeper.service.ts`, đăng ký trong `PaymentsModule` — nơi này đã import cả `BookingsModule` lẫn có `PaymentsService` riêng, tránh vòng lặp module `Bookings ↔ Payments`), gọi `PaymentsService.cancelOrderForBooking()` (seam đã có sẵn từ Phase 4) cho mỗi `bookingId` mà `sweep()` vừa expire. `BOOKING_CONFIRM_SLA_MINUTES` (env, default 30) giờ điều khiển `BookingsService.confirmPayment`'s `confirmDeadlineAt` — SLA thật, chạy trên mọi booking được IPN xác nhận thanh toán (`PaymentsService.markPaid` → `BookingsService.confirmPayment`), không còn phụ thuộc checkout giả lập nữa. `Reservation`/`ReservationStatus` (projection UI 6 giá trị, cả hai bản `shared/`) giữ nguyên như trước Phase 5 — phần mở rộng `awaiting_payment`/`expired`/`paymentStatus`/`holdExpiresAt`/`confirmDeadlineAt` viết ban đầu trên type đó bị revert lúc merge vì trùng lặp với `BookingRecord`/`BookingRecordStatus` (`api/src/features/bookings/booking.schema.ts`) Phase 2 đã có sẵn cho đúng mục đích này; `decisionNotification` (`booking.helpers.ts`) được mở rộng nhận `BookingRecordStatus` (thay vì chỉ `ReservationStatus`) + tham số `auto` để phát được cả copy auto-confirm lẫn hết-hạn-giữ-chỗ. Test: `api/test/bookings-sweeper.test.ts` (sweep rules), `api/test/bookings-service.test.ts` (confirmPayment's SLA-clock wiring, thay cho `venues-service.test.ts` cũ đã xoá).

### Phase 6 — Venue ops hardening (quyết định #11, #12, #15 + default CRM) - DONE
...
- **Ghi chú merge (2026-07-20):** nhánh Phase 6 (`eb7286f` api-ops + `47d9c49` web-ops) branch off `fc6ce7b` (gate Phase 3) và bị bỏ sót ở một lần orchestration trước; merge vào `master` (lúc này đã có Phase 4/5/7/8/9/10/11) resolve bằng cách port lại đúng delta của Phase 6 lên kiến trúc hiện tại thay vì lấy nguyên một bên: `venues.service.ts#composeBundle`/`withComputedStats` giữ cả `refundQueue` (Phase 4) lẫn `blocks ?? []`/`computeCustomerStats` (Phase 6); `shared/helpers.ts` nối tiếp cả hai đuôi hàm mới (Phase 10's `computeRevenueSeries`/`computeSportMix`/`computeChannelMix`/`computePeakHours`/`computeUtilizationHeatmap` rồi tới Phase 6's `computeCustomerStats`/`overlapsBlock`) thay vì chọn một bên; `bookings.service.ts createHold`/`createWalkIn` giữ nguyên seam `assertNoCourtBlock` thật (không còn no-op như lúc viết trên `fc6ce7b`) cộng thêm archived-court guard, không đụng gì tới Phase 4/5's hold/SLA logic đã landed sẵn ở đúng chỗ đó; `web/features/venue/venue-data-provider.tsx#courtDayEvents` giữ Phase 10's `isOwned`/fillerEnabled toggle cộng Phase 6's `blockDayEvents` overlay + "block thắng slot" logic. `NotificationKind`/`ReservationStatus` union types: giữ nguyên `master`'s `NOTIFICATION_KINDS` const-array hậu-Phase-7 thay vì Phase 6 nhánh's reformat thuần cosmetic. Gate xanh cả hai app sau merge (`pnpm typecheck && pnpm lint && pnpm test` ở `api`, `pnpm typecheck && pnpm lint && pnpm build` ở `web`).

### Phase 7 — Notification transactional (quyết định #14) - DONE
...
### Phase 8 — Vòng đời Stream chat (quyết định #13; SDK v9.x — có addMembers/removeMembers/updatePartial frozen) - DONE
...
### Phase 9 — Gỡ giả lập matchmaking (quyết định #10; phase rủi ro nhất) - DONE
...
### Phase 10 — Analytics trung thực - DONE
...
### Phase 11 — Tài liệu + release checklist - DONE

- Cập nhật `VienTD-Review/`: thêm mục "Quyết định đã chốt (2026-07-13)" cho từng doc (8/8 flow doc + README) với 16 quyết định + defaults, trả lời trực tiếp "Quyết định cần xác nhận" của từng doc; đính chính các claim lỗi thời (catalog đã hợp nhất, `TODAY_ISO` không còn, cross-write player↔venue đã tồn tại từ trước) trực tiếp trong `README.md` và các doc 05/08 liên quan; đánh dấu DONE/CHƯA LÀM cho từng phase ở mục 3 phía trên (xác nhận bằng `git log`: chỉ Phase 0 và Phase 1 đã merge, cùng commit account-type onboarding trước đó không thuộc roadmap phase nào nhưng đã chốt là "đã commit" ở mục 1).
- Release checklist: `RELEASE.md` (repo root) — env hiện tại (`api/.env.example`, `web/.env.example`) + env `SEPAY_*` mục tiêu cho Phase 4 (chưa implement, đánh dấu rõ trạng thái); setup tunnel IPN (ngrok/cloudflared) + quy trình test trên sandbox SePay; quy trình reseed (drop collection, seed-on-first-read); ma trận `pnpm typecheck && pnpm lint && pnpm test && pnpm build` cho cả hai app; ghi chú Clerk Test Mode E2E (`+clerk_test`, mã `424242`) theo mục 5 của roadmap.
- **Ghi chú lúc merge (2026-07-20):** nhánh Phase 11 được viết trên base `cc3a033`, thời điểm đó `git log` xác nhận đúng là chỉ Phase 0/1 đã merge nên bảng trạng thái ở mục 3 đánh dấu Phase 2–10 "CHƯA LÀM" toàn bộ. Trong lúc chờ merge, các phase 2/3/4/5/7/8/9/10 đã lần lượt land song song vào `master` (xem `git log cc3a033..master`); lúc resolve conflict, trạng thái các phase đó ở mục 3 được cập nhật lại thành DONE cho khớp `git log` thật (chỉ Phase 6 — venue ops hardening — còn CHƯA LÀM). Nội dung mô tả kế hoạch dưới mỗi phase giữ nguyên như viết ban đầu (tương tự cách Phase 5 đã làm với ghi chú triển khai riêng của nó); RELEASE.md và env `SEPAY_*` ở trên đã đúng thực tế Phase 4 (SePay checkout/IPN thật đã DONE, không còn "chưa implement" — flag đó chỉ đúng tại thời điểm viết Phase 11, không sửa lại RELEASE.md trong lần merge này vì nằm ngoài phạm vi Phase 11 gốc, cần review riêng).
