    ...
    <Accordion.Root className={cn("flex flex-col gap-3.5", className)}>
      {items.map((item, i) => (
        <Accordion.Item
          key={i}
          value={i}
          className="group overflow-hidden rounded-2xl border border-border/80 bg-card/80 shadow-xs backdrop-blur-sm transition-all duration-300 hover:border-primary/40 data-[panel-open]:border-primary/50 data-[panel-open]:bg-card data-[panel-open]:shadow-md"
        >
          <Accordion.Header className="m-0">
            <Accordion.Trigger className="flex w-full cursor-pointer items-center justify-between gap-4 px-6 py-5 text-left text-base font-semibold text-foreground focus-visible:ring-3 focus-visible:ring-ring/40 focus-visible:outline-none sm:text-lg">
              <div className="flex items-center gap-3.5">
                <span className="flex size-7 shrink-0 items-center justify-center rounded-lg bg-primary/10 font-mono text-xs font-bold text-primary transition-colors duration-300 group-data-[panel-open]:bg-lime group-data-[panel-open]:text-lime-foreground">
                  0{i + 1}
                </span>
                <span className="transition-colors duration-300 group-data-[panel-open]:text-primary">
                  {item.question}
                </span>
              </div>
