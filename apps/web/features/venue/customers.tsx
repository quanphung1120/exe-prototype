...
export function VenueCustomersView({
  ...
  const { venueCustomers: VENUE_CUSTOMERS, addCustomer } = useVenueData()
        ...
        action={
          <div className="flex items-center gap-3">
            ...
            >
              ...
              {locale === "vi" ? "Thêm khách" : "Add Customer"}
            </Button>
          </div>
        }
      >
...
function AddCustomerDialog({ isOpen, onClose, onAdd, locale, customersList }: AddCustomerDialogProps) {
  ...
  const copy = locale === "vi"
    ? {
        ...
        description: "Nhập thông tin khách hàng mới. Số điện thoại sẽ dùng làm ID duy nhất.",
        name: "Tên khách hàng",
        phone: "Số điện thoại (ID)",
        sport: "Môn thể thao yêu thích",
        cancel: "Hủy",
        submit: "Thêm",
        saving: "Đang lưu...",
      }
    : {
        title: "Add Customer",
        description: "Enter the new customer details. The phone number will serve as their unique ID.",
        name: "Customer Name",
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      ...
      if (exists) {
        setError(locale === "vi" ? "Số điện thoại này đã tồn tại trong danh sách!" : "A customer with this phone number already exists!")
        return
      }

      const initials = trimmedName.split(" ").map(w => w[0]).filter(Boolean).join("").toUpperCase().slice(0, 2)

      const newCustomer: VenueCustomer = {
        id: trimmedPhone,
        name: trimmedName,
        initials: initials || "??",
        favoriteSport: sport,
      ...
      onAdd(newCustomer)
      toast.success(locale === "vi" ? "Đã thêm khách hàng thành công" : "Customer added successfully")
      setName("")
      setPhone("")
      onClose()
    })
